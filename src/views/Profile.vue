...
import { 
  ...
  IconInfoCircle, IconRefresh, IconCamera
...
watch(form, () => {
  ...
  saveTimer = setTimeout(performSave, 1000) // 1 秒防抖
}, { deep: true })
...
const removeTeam = (name: string) => {
  ...
  if (index !== -1) form.teams.splice(index, 1)
}
                  ...
                  <ARow :gutter="24">
                    <ACol :span="24" :lg="12">
                      <AFormItem label="显示名称" help="修改后自动保存">
                        <AInput v-model="form.userName" placeholder="请输入您的名字" size="large">
